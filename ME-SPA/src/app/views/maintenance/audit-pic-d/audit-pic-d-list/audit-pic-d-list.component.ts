...
export class AuditPicDListComponent implements OnInit {
  ...
  load() {
    if (this.searchKey === false) {
      this.auditPicDService.getListAll(this.pagination.currentPage, this.pagination.itemsPerPage)
      .subscribe((res: PaginatedResult<AuditPicD[]>) => {
      ...
      this.auditPicDService.search(this.pagination.currentPage, this.pagination.itemsPerPage, this.text)
        .subscribe((res: PaginatedResult<AuditPicD[]>) => {
          this.auditPics = res.result;
          this.pagination = res.pagination;
        }, error => {
