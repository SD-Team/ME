...
namespace ME_API._Services.Services
{
    public class AuditRateService : IAuditRateService
    {
        ...
        public async Task<object> GetMEPIC()
        {
            var result = await _auditPicDRepository.FindAll(x => x.PIC_Type_ID == "1").Select(x => new { x.Resp_ID, x.Resp_ZW }).ToListAsync();
            return result;
        }

        public async Task<object> GetPDRESP()
        {
            var result = await _auditPicDRepository.FindAll(x => x.PIC_Type_ID == "2").Select(x => new { x.Resp_ID, x.Resp_ZW }).ToListAsync();
            return result;
        }
